...
export function useTailors() {
  ...
  const saveTailors = useCallback((updated: TailorProfile[]) => {
    setTailors(updated);
    try { localStorage.setItem(TAILORS_KEY, JSON.stringify(updated)); } catch {}
  }, []);
  ...
  const createTailorProfile = useCallback((profile: Omit<TailorProfile, 'id' | 'principalId' | 'createdAt' | 'isApproved'>) => {
  ...
  const updateTailorProfile = useCallback((id: string, updates: Partial<TailorProfile>) => {
    saveTailors(tailors.map(t => t.id === id ? { ...t, ...updates } : t));
  }, [tailors, saveTailors]);

  const approvedTailors = tailors.filter(t => t.isApproved);

  return { tailors, approvedTailors, getMyTailorProfile, createTailorProfile, updateTailorProfile };
}
