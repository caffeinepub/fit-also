...
export function useMeasurements() {
  ...
  const save = useCallback((updated: MeasurementProfile[]) => {
    setProfiles(updated);
    try { localStorage.setItem(getMeasKey(principalId), JSON.stringify(updated)); } catch {}
  }, [principalId]);

  const addProfile = useCallback((profile: Omit<MeasurementProfile, 'id' | 'createdAt'>) => {
  ...
  const updateProfile = useCallback((id: string, updates: Partial<MeasurementProfile>) => {
