...
export function MeasurementProfileSelector({ value, onChange }: MeasurementProfileSelectorProps) {
  const { t } = useLanguage();
  ...
  return (
    <div className="grid gap-1.5">
      ...
      {profiles.length === 0 && (
        <p className="text-xs text-muted-foreground">No measurement profiles found. Please add one in your dashboard.</p>
      )}
    </div>
  );
}
