...
const CITIES = ['All Cities', 'Mumbai', 'Delhi', 'Jaipur', 'Bangalore', 'Chennai', 'Kolkata', 'Hyderabad', 'Pune'];
const SPECIALTIES = ['All Specialties', 'Shirts', 'Kurtas', 'Suits', 'Sherwanis', 'Trousers', 'Lehengas', 'Saree Blouses', 'Anarkalis'];
...
export function TailorDirectoryPage() {
  ...
  const filtered = approvedTailors.filter(tailor => {
    const matchSearch = !search || tailor.shopName.toLowerCase().includes(search.toLowerCase());
    const matchCity = city === 'All Cities' || tailor.city === city;
    const matchSpecialty = specialty === 'All Specialties' || tailor.specialties.includes(specialty);
  ...
  return (
    <div className="container mx-auto px-4 py-10 animate-fade-in">
      <div className="mb-8">
        <p className="text-accent font-medium text-sm tracking-widest uppercase mb-1">Artisans</p>
        <h1 className="font-serif text-3xl md:text-4xl font-bold text-foreground">{t('tailor.directory')}</h1>
      </div>
            ...
            >
              ...
              <div className="flex items-center justify-between text-sm border-t border-border pt-4">
                <div className="flex items-center gap-1 text-muted-foreground">
                  ...
                  <span>{tailor.turnaroundDays} {t('common.days')}</span>
                </div>
                <span className="font-semibold text-primary">From ₹{tailor.basePricing.toLocaleString()}</span>
              </div>
            </LuxuryCard>
          ))}
        </div>
