...
import { Package, Store, List, TrendingUp } from 'lucide-react';
...
export function TailorDashboard() {
  ...
  return (
    <div className="container mx-auto px-4 py-10 animate-fade-in">
      <div className="mb-8">
        <p className="text-accent font-medium text-sm tracking-widest uppercase mb-1">Tailor Portal</p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        {[
          ...
          { label: 'Active Orders', value: orders.filter(o => !['delivered', 'cancelled'].includes(o.status)).length, icon: List },
          { label: 'Delivered', value: orders.filter(o => o.status === 'delivered').length, icon: TrendingUp },
              ...
              {orders.map(order => (
                <LuxuryCard key={order.id} className="p-5">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                      <h3 className="font-serif font-semibold">{order.listingTitle}</h3>
                      <p className="text-sm text-muted-foreground font-mono">{order.id}</p>
                      <p className="text-xs text-muted-foreground mt-1">
                        {new Date(order.createdAt).toLocaleDateString()} · ₹{order.price.toLocaleString()}
                      </p>
                    </div>
