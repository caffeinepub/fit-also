...
import { ArrowLeft, MapPin, Clock, Scissors, ExternalLink } from 'lucide-react';
  ...
  if (!tailor) {
    return (
      <div className="container mx-auto px-4 py-20 text-center">
        ...
        <LuxuryButton variant="outline" size="sm" className="mt-4" onClick={() => navigate({ to: '/tailors' })}>
          Back to Directory
        </LuxuryButton>
      </div>
    );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <LuxuryCard className="p-6">
            <div className="flex flex-col items-center text-center mb-6">
              ...
              <h1 className="font-serif text-2xl font-bold text-foreground">{tailor.shopName}</h1>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between p-3 rounded-lg bg-muted">
                ...
                <span className="font-semibold text-sm">{tailor.turnaroundDays} days</span>
              </div>
              <div className="flex items-center justify-between p-3 rounded-lg bg-muted">
                <span className="text-sm text-muted-foreground">Starting from</span>
                <span className="font-bold text-primary">₹{tailor.basePricing.toLocaleString()}</span>
              </div>
            </div>
          ...
          {tailor.portfolioUrls.filter(u => u).length > 0 && (
            <LuxuryCard className="p-5">
              ...
              <div className="space-y-2">
                {tailor.portfolioUrls.filter(u => u).map((url, i) => (
                  <a
                    key={i}
                    href={url}
                    target="_blank"
        ...
        <div className="lg:col-span-2 space-y-6">
          <LuxuryCard className="p-6">
            ...
            <p className="text-muted-foreground leading-relaxed">{tailor.bio || 'No bio provided.'}</p>
          </LuxuryCard>

          {tailorListings.length > 0 && (
            <div>
              <h2 className="font-serif text-xl font-semibold mb-4">Available Listings</h2>
                ...
                {tailorListings.map(listing => (
                  ...
                  >
                    ...
                    <div className="p-4">
                      ...
                      <h3 className="font-serif font-semibold text-foreground mt-0.5">{listing.title}</h3>
                      <p className="font-bold text-primary font-serif mt-1">₹{listing.basePrice.toLocaleString()}</p>
                    </div>
                  </LuxuryCard>
                ))}
              </div>
