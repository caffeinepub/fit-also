...
const CATEGORIES: GarmentCategory[] = [
  'Shirts', 'Kurtas', 'Suits', 'Sherwanis', 'Trousers', 'Lehengas', 'Saree Blouses', 'Anarkalis'
];
...
export function CatalogPage() {
  ...
  return (
    <div className="container mx-auto px-4 py-10 animate-fade-in">
      {/* Header */}
      <div className="mb-8">
        <p className="text-accent font-medium text-sm tracking-widest uppercase mb-1">Explore</p>
        <h1 className="font-serif text-3xl md:text-4xl font-bold text-foreground">{t('catalog.title')}</h1>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {listings.map(listing => (
            ...
            >
              ...
              <div className="p-4">
                <h3 className="font-serif font-semibold text-foreground mb-1 line-clamp-1">{listing.title}</h3>
                <div className="flex items-center gap-1 text-xs text-muted-foreground mb-3">
                  <MapPin className="h-3 w-3" />
                  <span>{t('catalog.by')} {listing.tailorName}, {listing.tailorCity}</span>
                </div>
                <div className="flex items-center justify-between">
                  <div>
                    <span className="text-xs text-muted-foreground">{t('listing.basePrice')}</span>
                    <div className="font-bold text-primary font-serif">₹{listing.basePrice.toLocaleString()}</div>
                  </div>
                  <div className="flex items-center gap-1 text-xs text-muted-foreground">
                    <Clock className="h-3 w-3" />
                    <span>{listing.estimatedDays} {t('common.days')}</span>
                  </div>
                </div>
              </div>
            </LuxuryCard>
